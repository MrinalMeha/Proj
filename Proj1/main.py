...
def convertFromFormat2 (jsonObject):
    iso_time = jsonObject['timestamp']
    dt = datetime.fromisoformat(iso_time.replace('Z', '+00:00'))
    ms_time = int(dt.timestamp() * 1000)

    # IMPLEMENT: Conversion From Type 1
    # convert the ISO 8601 timestamp to milliseconds since epoch
    # data= datetime.strptime(jsonObject['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ') #ISO 8601 format
    # timestamp = round((data - datetime(1970, 1, 1)).total_seconds() * 1000) #convert to milliseconds since epoch

    #create a new dictionary for the unified format
    result={
        'deviceID': jsonObject['device']['id'],  #extract the device ID
        'deviceType': jsonObject['device']['type'], #extract the device type
        'timestamp': ms_time, #use the converted timestamp
        'location': {
